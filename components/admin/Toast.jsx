...
function ToastItem({ toast, onRemove }) {
  ...
  return (
    <div
      className={`flex items-center gap-3 px-5 py-3.5 rounded-2xl shadow-lg text-white text-sm font-medium transition-all duration-300 ${bgColor} ${
        isLeaving
          ? "opacity-0 translate-x-8"
          : "opacity-100 translate-x-0"
      }`}
      style={{ animation: isLeaving ? "none" : "slideIn 0.3s ease-out" }}
    >
      {icon}
      <span className="flex-1">{toast.message}</span>
      <button
        onClick={() => { setIsLeaving(true); setTimeout(onRemove, 300); }}
